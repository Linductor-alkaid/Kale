/**
 * @file test_lighting_pass.cpp
 * @brief phase8-8.6 Lighting Pass 单元测试
 *
 * 覆盖：SetupLightingPass 声明 Lighting、ReadTexture(GBuffer*3 + ShadowMap)、WriteColor(0)；
 * Compile 成功；拓扑序 Shadow → GBuffer → Lighting；Execute 不崩溃；
 * 仅 SetupLightingPass 时单 Pass Compile。
 */
...
static void test_lighting_pass_setup_and_compile_full_chain() {
    ...
    TEST_CHECK(order.size() == 3u);  // Shadow → GBuffer → Lighting

    const auto& passInfo = rg.GetCompiledPassInfo();
    TEST_CHECK(passInfo.size() == 3u);
    std::size_t lightingPassIdx = 2u;
    const auto& lightingInfo = passInfo[lightingPassIdx];
    TEST_CHECK(lightingInfo.colorOutputs.size() == 1u);
    TEST_CHECK(lightingInfo.depthOutput == kale::pipeline::kInvalidRGResourceHandle);
    TEST_CHECK(!lightingInfo.writesSwapchain);
    TEST_CHECK(lightingInfo.readTextures.size() == 4u);  // albedo, normal, depth, shadowMap

    kale_device::TextureHandle lightingTex = rg.GetCompiledTexture(rg.GetHandleByName("Lighting"));
    TEST_CHECK(lightingTex.IsValid());
}

static void test_lighting_pass_execute_no_crash() {
    kale::pipeline::RenderGraph rg;
...
static void test_lighting_pass_only() {
    kale::pipeline::RenderGraph rg;
...
int main() {
    test_lighting_pass_setup_and_compile_full_chain();
    test_lighting_pass_execute_no_crash();
    test_lighting_pass_only();
    std::cout << "test_lighting_pass OK\n";
    return 0;
}
